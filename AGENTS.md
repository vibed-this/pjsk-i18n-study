...
**任务队列**见根目录 [TODO.md](TODO.md)（待办、进行中、已完成、搁置）。

---

## 任务队列（TODO.md）

[TODO.md](TODO.md) 是项目的**单一任务来源**，记录后续待办、当前焦点与已完成项。

### 强制规则

1. **分析前**：阅读 [TODO.md](TODO.md)，确认当前焦点与相关待办，避免重复劳动或偏离优先级。
2. **产生新待办时**（新 Hook 点、新阻塞、新验证项、笔记待同步、版本更新等）：**必须先更新 TODO.md，再继续当前技术工作**。
   - 新任务写入对应优先级段落（P0–P5），标注 `[ ]` 或 `[~]`
   - 已完成项移至「已完成」，原条目标 `[x]`
   - 搁置/证伪方案移至「搁置 / 阻塞」，标注 `[-]` 并简述原因
   - 若焦点转移，更新「当前焦点」段落
3. **会话结束前**：核对 TODO 是否与本次结论一致；若笔记中写了「下一步」而 TODO 未体现，补录到 TODO。

> TODO 记「做什么」；`notes/` 记「怎么做、结论是什么」。二者互补，不可互相替代。
...
1. **分析前**：阅读 [TODO.md](TODO.md) 的「当前焦点」与相关待办。
2. **分析中**：一旦产生新的待办、阻塞或优先级变化，**立即**更新 TODO.md，然后再继续原任务。
3. **分析后**：将本次关闭的待办标为 `[x]`；未完成的保留或拆分为更细条目。
...
- [ ] 已阅读并同步 [TODO.md](TODO.md)（新待办已写入、已完成项已勾选、焦点已更新）
