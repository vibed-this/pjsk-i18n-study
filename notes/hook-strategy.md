...
1. **Frida 收尾**：主界面菜单 `intercept`；`TMP_Text.set_text` 主界面行为确认。
2. **Zygisk 模块**：参照 gakuen-imas-localify，已验证 IDA 偏移 + 混合 resolve 策略封装 ShadowHook/Dobby。
3. **翻译数据**：从 sekai.best 同步 scenario JSON 与 MasterWording 词表，建立 key/原文 → 中文映射。
4. **版本维护**：在 `offsets.js` 或配置中标注 `versionName`；落地 P6 重验证 checklist。
